...
> & {
  ...
  user: Pick<typeof userTable.$inferSelect, "id" | "name" | "imgUrl"> & {
    resume:
...
function getColumns(
  ...
  return [
    ...
    },
    {
      ...
      sortingFn: ({ original: a }: any, { original: b }: any) => {
        return sortApplicationsByStage(a.stage, b.stage)
      },
      filterFn: ({ original }: any, _: any, value: any) => {
    ...
    },
    {
      ...
      cell: ({ row }: any) => row.original.createdAt.toLocaleDateString(),
    },
    {
      id: "actions",
      cell: ({ row }: any) => {
  ...
  return (
    <DataTable
      ...
      initialFilters={[
        {
          id: "stage",
          value: applicationStatuses.filter(stage => stage !== "rejected"),
        },
      ]}
    />
  )
...
function StageCell({
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent>
        {applicationStatuses.toSorted(sortApplicationsByStage).map(stageValue => (
          <DropdownMenuItem
            key={stageValue}
            onClick={() => {
...
function StageDetails({ stage }: { stage: ApplicationStatus }) {
  return (
